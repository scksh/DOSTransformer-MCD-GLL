...
def parse_args():
    ...
    parser.add_argument("--es", type=int, default=5, help="Early Stopping Criteria")
    ...
    parser.add_argument("--attn_drop", type=float, default=0.1, help = 'attention dropout ratio')
...
def test_phonon(model, data_loader, criterion, r2, device):
    ...
    return loss_rmse_sys/(bc + 1), loss_mse_sys/(bc+1), loss_mae_sys/(bc+1), loss_r2_sys/(bc+1), preds_system
...
#     edge_batch = positions.new_zeros(positions.shape[0], dtype=torch.long)[torch.from_numpy(edge_src).long()]
#     # edge_batch = positions.new_zeros(positions.shape[0], dtype=torch.long)[torch.from_numpy(edge_src)]
#     # edge_vec = (positions[torch.from_numpy(edge_dst)]
#     #             - positions[torch.from_numpy(edge_src)]
#     #             + torch.einsum('ni,nij->nj', torch.tensor(edge_shift, dtype=default_dtype), lattice[edge_batch]))
#     edge_vec = (positions[torch.from_numpy(edge_dst).long()]
#                 - positions[torch.from_numpy(edge_src).long()]
#                 + torch.einsum('ni,nij->nj', torch.tensor(edge_shift, dtype=default_dtype), lattice[edge_batch]))
...
def build_data(entry, r_max=5.):
    ...
    system = system_dict.get(entry.get('crystal_system', 'Unknown'), 6)
