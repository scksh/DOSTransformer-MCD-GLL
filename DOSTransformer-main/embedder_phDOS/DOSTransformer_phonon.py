...
class DOSTransformer_phonon(nn.Module):
    ...
    def forward(self, g):
        ...
        # 모든 텐서를 GPU로 이동
        input_ids = torch.arange(51, device=self.device)  # Energies 임베딩에 사용
        token_ids = torch.arange(7, device=self.device)   # Prompt 토큰에 사용
    
        energies = self.embeddings(input_ids)
        prompt_token = self.prompt_token(token_ids)
    
        # `preprocess` 단계에서 생성된 edge_vec, edge_sh, edge_attr를 GPU로 이동
        edge_vec = self.preprocess(g).to(self.device)
        edge_sh = o3.spherical_harmonics(o3.Irreps.spherical_harmonics(1), edge_vec, True, normalization='component').to(self.device)
        edge_length = edge_vec.norm(dim=1).to(self.device)
        edge_attr = (smooth_cutoff(edge_length / 4.0).to(self.device)[:, None] * edge_sh).to(self.device)
    
        # GNN 인코더 호출 시 입력도 GPU에 위치해야 함
        x, edge_attr, energies = self.GN_encoder(
            x=g.x.to(self.device),
            edge_attr=edge_attr,
            batch=g.batch.to(self.device),
            energies=energies,
        )
    
        # Processor 및 Transformer에 입력되는 데이터도 GPU에서 동작하도록 설정
        for processor in self.stacked_processor:
            out_x, out_edge_attr = processor(
                x=x, edge_index=g.edge_index.to(self.device), edge_attr=edge_attr
            )
            x = x + out_x
            edge_attr = edge_attr + out_edge_attr
    
        # Transformer에서 처리
        x_dense, _ = to_dense_batch(x, batch=g.batch.to(self.device))
        x_dense = x_dense.transpose(0, 1)  # Transpose to (Seq, Batch, Feature)
        energies = self.transformer(energies, x_dense, x_dense)
    
        # GN 디코더
        graph = self.GN_decoder(x, g.batch.to(self.device))
        graph = graph.reshape(-1, graph.shape[0], graph.shape[1]).expand(51, graph.shape[0], graph.shape[1])
        ...
        dos_global = self.transformer_self(dos_input, dos_input, dos_input)
        dos_global = self.transformer_source(dos_global, x_dense, x_dense)
        dos_global = self.out_layer(dos_global).squeeze(2).T
    
        # Prompt 토큰 사용
        prompt_token = prompt_token[g.system].repeat(51, 1, 1).to(self.device)
        dos_input = torch.cat([energies, graph, prompt_token], dim=2)
        dos_input = F.leaky_relu(self.fc_prompt(dos_input))
        dos_system = self.transformer_self(dos_input, dos_input, dos_input)
        dos_system = self.transformer_source(dos_system, x_dense, x_dense)
        dos_system = self.out_layer(dos_system).squeeze(2).T
